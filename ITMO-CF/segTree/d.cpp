...
public :
	...
	int getUtil(int si, int sl, int sr, int ql, int val){
		// not vvalid;
		if(sr < ql) return -1;
		...
		// recurrsion;
		int mid = getMid(sl, sr);
		int leftAns = getUtil(lsi, sl, mid, ql, val);
		if(leftAns >= 0){
			return leftAns;
		}
		else{
			return getUtil(rsi, mid + 1, sr, ql, val);
		}
	}
	int getNotLess(int ql, int val){
		return getUtil(0, 0, n - 1, ql, val);
	}
};
